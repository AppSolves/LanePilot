...
def start_network(tcp_port: int, udp_port: int) -> None:
    ...
    signal.signal(
        signal.SIGTERM,
        lambda _, __: stop_threads(
            [gstreamer_thread, server_thread, servo_manager_thread]
        ),
    )
    gstreamer_thread.join()
    stop_threads([gstreamer_thread, server_thread, servo_manager_thread])

    # After joining, check for exceptions
    for t in [server_thread, gstreamer_thread, servo_manager_thread]:
